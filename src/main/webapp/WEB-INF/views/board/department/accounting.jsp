      ...
      <!--sidebar end-->
		...
		<section id="main-content">
			<section class="wrapper">
				<h3>
					<i class="fa fa-angle-right"></i> 부서 게시판 (경리부)
				</h3>
				<hr>
				<div align="center"><a href="depWrite" style="float: left"><button type="button" class="btn btn-sm btn-success">글쓰기</button></a>
				<nav >
				<ul>
							...
							<table cellpadding="0" cellspacing="0" border="0"
								...
								<thead>
									<tr>
										...
										<th><center>작성일자</center></th>
									</tr>
								</thead>
								<tbody>
									<c:forEach items="${accounting }" var="dep">
										<tr class="gradeC">
											<td><a href="depView?bno=${dep.bno }">${dep.title }</a></td>
											<td>${dep.writer }</td>
											<td>${dep.department }</td>
											<td><fmt:formatDate value="${dep.regdate }" pattern="yyyy-MM-dd (E) HH:mm:ss (a)"/></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
